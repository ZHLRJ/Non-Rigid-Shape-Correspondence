             ...
             std::vector<std::array<double, 3>>& edgescolor,std::vector<SurfacePoint> &path)
{
    ...
    for (size_t i=0;i<path.size()-1;++i){
        ...
        tempNode=path[i].interpolate(geometry->vertexPositions);
...
//    }
    ...
    if (ImGui::Button("drawEmanationMMP"))
    {
        ...
        // visualize all geopath with random color
        polyscope::CurveNetwork* allGeoPath=polyscope::registerCurveNetwork("geopath", nodes, edges);
        allGeoPath->setEnabled(true);
        allGeoPath->addEdgeColorQuantity("random color", edgescolor);
...
//    std::string filepath = "../../../../input/T-RexSkull_Skull.obj";
    std::string filepath ="../../../../input/smallDataset/horse0.obj";
    if (inputFilename) {
